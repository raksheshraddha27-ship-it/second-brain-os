...
resolve on its own. This exists because chat output and a one-line `log.md` entry aren't enough:
a scheduled/headless sync can flag something with nobody watching, and it needs to still be
...
2. If the resolution requires a wiki edit (e.g. adding a link, updating a page to remove the
   contradiction), make that edit now, following `CLAUDE.md`'s conventions - this is the one
...
5. **Write a learning.** A human resolving a flagged item is exactly the kind of decision future
   ...
   direct page edits: `<node-name>-<date>.md`) summarizing the resolution, so `wiki-ingest` picks
   it up on the next run and doesn't reintroduce the same contradiction or leave a similar page
   disconnected.
...
- Don't silently skip old items - if something has been pending a long time, mention its age when
  listing, but don't auto-resolve or auto-archive it (staleness archiving in `lint` only applies
  to wiki pages, not review findings).
